...
(function () {
  function inDrawerMode() {
    ...
    return window.matchMedia("(min-width: 721px) and (max-width: 980px)").matches;
  }

  function ensureBackdrop() {
    let el = document.querySelector(".nav-drawer-backdrop");
    if (!el) {
      el = document.createElement("div");
      el.className = "nav-drawer-backdrop";
  ...
  function openDrawer(mobileMenu, hamburger, backdrop) {
    mobileMenu.setAttribute("aria-hidden", "false");
    hamburger.setAttribute("aria-expanded", "true");
    backdrop.classList.add("is-open");
    backdrop.setAttribute("aria-hidden", "false");

    // prevent body scroll while drawer is open (mid-width only)
    document.documentElement.style.overflow = "hidden";
    document.body.style.overflow = "hidden";
  }

  function closeDrawer(mobileMenu, hamburger, backdrop) {
    mobileMenu.setAttribute("aria-hidden", "true");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    backdrop.addEventListener("click", () => {
      ...
      closeDrawer(mobileMenu, hamburger, backdrop);
    });
    ...
    // Hook hamburger behavior WITHOUT breaking your existing phone behavior:
    // We only manage backdrop + scroll lock in drawer mode.
    hamburger.addEventListener("click", () => {
      if (!inDrawerMode()) return;
      ...
      if (isOpen) {
        ...
        openDrawer(mobileMenu, hamburger, backdrop);
      }
    });

    // If viewport changes while drawer open, close + cleanup
    window.addEventListener("resize", () => {
      const isOpen = mobileMenu.getAttribute("aria-hidden") === "false";
      if (!isOpen) return;

      // if leaving drawer mode, remove overlay + unlock scroll
      if (!inDrawerMode()) {
        closeDrawer(mobileMenu, hamburger, backdrop);
      }
    });
